...
bot = telebot.TeleBot(
    "6206215288:AAHyC9MNEV4JmfHSe-HwGyyYgsfiri_xKRo", parse_mode=None)
...
def send_welcome(message):
    name = input("اسمت چیه؟")
    bot.reply_to(
        message, "سلام % خوش امدی.هر راهنمایی خواستی فقط HELP رو بزن", name)
...
def qrcode1(message):
    message = bot.send_message(
        message, "یه چیزی بگو تا برات تبدیلش کنم به QRcode:  ")
...
def age1(message):
    message = bot.send_message(
        message.chat.id, "تاریخ تولد خود را وارد نمایید: مثال: 17/11/1364")
...
def age(message):
    ...
    age = JalaliDatetime.now(
    )-JalaliDatetime(birth_lst[0], birth_lst[1], birth_lst[2])//365
...
def voice1(message):
    message = bot.send_message(
        message.chat.id, "یک جمله به زبان انگلیسی وارد نمایید: ")
